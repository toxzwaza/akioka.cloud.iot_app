...
class AcceptController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // user_idによって処理を振り分ける
        $query = DB::table('order_requests')->select('order_requests.id', 'order_requests.stock_id', 'order_requests.id as order_request_id', 'order_requests.accept_flg', 'stocks.img_path', 'stocks.name', 'stocks.s_name', 'stocks.price as stock_price', 'stocks.url', 'order_requests.quantity', 'order_requests.created_at', 'users.name as request_user_name', 'order_requests.postage', 'order_requests.calc_price', 'order_requests.price', 'suppliers.name as supplier_name', 'users.name as request_user_name')
